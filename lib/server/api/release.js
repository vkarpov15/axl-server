...
exports.create = function(Release, Project, PipeToS3,
  config, CloudfrontConverter) {
  return function(req, res) {
    var project = req.query.project.toString();
    var version = req.query.version.toString();
    ...
    Project.findOneAndUpdate(
      ...
      function(error, project) {
        ...
        var maintainerUsernames =
          _.pluck(project.data.maintainers, 'username');
        if (maintainerUsernames.indexOf(req.user.username) === -1) {
          return res.
            status(status.UNAUTHORIZED).
            json({ redirect: '/auth/twitter' });
        }

        if (project.releases.indexOf(version) != -1) {
          var err = 'Cant overwrite existing release ' +
            release.stringify;
          return res.
